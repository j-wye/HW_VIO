...
void Updater::mscUpdate(MSCEqFState& X, const Tracks& tracks, std::unordered_set<uint>& ids)
{
  ...
  for (const auto& id : ids)
  {
    ...
    // A track may reference timestamps for which no clone exists: the tracker starts
    // accumulating observations before the filter is initialised and keeps them across
    // frames where no clone was created (propagation failure, zero-velocity early
    // return). X.clone(t) would throw std::out_of_range there, so skip such tracks.
    if (std::any_of(track.timestamps_.begin(), track.timestamps_.end(),
                    [&X](const fp& t) { return X.clones_.find(t) == X.clones_.end(); }))
    {
      utils::Logger::debug("Track with id: " + std::to_string(id) +
                           " references a timestamp with no clone. Skipping.");
      continue;
    }
